...
def _translate_segments_llm(
    ...
    def _is_dialogue(seg) -> bool:
        ...
        if not text:
            return False
        # v305 (5/26): SPEAKER_BG_* (gap-fill 배경 화자) 는 영어 원본 외침 그대로 유지
        if str(getattr(seg, "speaker", "")).startswith("SPEAKER_BG"):
            return False
...
def run_pipeline(
    ...
    for chunk_name, data in chunk_data.items():
        ...
        # v300 (5/25): words.json dump — 후처리 word-level text reassign용.
        # 메인 orchestrator는 화자 segment에 ASR utterance를 통째 복사하는 bug 있음 →
        # 별도 후처리 script가 이 words + segments.json 결합해 text 재배치.
        try:
            from pathlib import Path as _Pw
            _md = _Pw(RUNS_DIR) / CURRENT_RUN_ID / "meta"
            _md.mkdir(parents=True, exist_ok=True)
            _wj = _md / f"{chunk_name}_words.json"
            with open(_wj, "w", encoding="utf-8") as _wf:
                json.dump({"chunk_name": chunk_name,
                           "detected_lang": data["detected_lang"],
                           "words": [{"word": w.word,
                                      "start": float(w.start),
                                      "end": float(w.end)} for w in words]},
                          _wf, ensure_ascii=False, indent=2)
            print(f"[ASR] words.json 저장: {_wj} ({len(words)} words)")
        except Exception as _ew:
            print(f"[ASR] words.json 저장 실패: {_ew}")
    ...
    for chunk_name, data in chunk_data.items():
        ...
        # v305 (5/26): word-level text reassign — intra-segment split 으로 같은 utterance 가
        # 여러 segment 에 통째 복사되는 bug fix (test5 마지막 'Hello Doctor Sean Murphy...' 3중복 등).
        # words 시간 기준으로 각 segment 에 해당 단어만 모아 text 재구성.
        words_for_chunk = data.get("words", [])
        if words_for_chunk:
            n_fixed = 0
            for seg in data["segments"]:
                margin = 0.05
                picked = []
                for w in words_for_chunk:
                    try:
                        c = (float(w.start) + float(w.end)) / 2.0
                    except Exception:
                        continue
                    if seg.start - margin <= c <= seg.end + margin:
                        picked.append(w.word)
                new_text = " ".join(picked).strip()
                if new_text and new_text != seg.text:
                    print(f"[WordReassign] seg{seg.id} {seg.text[:40]!r} -> {new_text[:40]!r}")
                    seg.text = new_text
                    n_fixed += 1
            if n_fixed:
                print(f"[WordReassign] {n_fixed}/{len(data['segments'])} segments text 재할당")
