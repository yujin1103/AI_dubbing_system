...
def reassign_segments_by_face_voting(
    ...
    Args:
        audio_segments: [(start, end, speaker), ...]
        asd_result: run_asd 결과
        track_embeddings: compute_track_face_embeddings 반환
        speaker_face_count: fusion["speaker_face_count"] (face_id remap 후 권장)
        similarity_threshold: best speaker의 cosine sim 최소값
        margin_threshold: 2위 speaker와의 sim 차이 최소값 (모호 케이스 회피)
        min_seg_face_share: segment 길이 중 face frame 비율 최소값
        max_seg_duration: None=무제한; 값이 있으면 그보다 긴 segment skip

    Returns: [(start, end, old_speaker, new_speaker), ...]

    환경변수 (orchestrator에서 override):
    ...
    # 1) 각 speaker의 대표 face embedding (frame-수 가중 평균)
    speaker_ref_emb: Dict[str, np.ndarray] = {}
    ...
    if verbose:
        ...
        # pairwise similarity matrix (debug)
        ...
        for i, a in enumerate(spk_list):
            for b in spk_list[i + 1:]:
                ...
                if s >= 0.50:  # noisy pair만 출력
                    print(f"  {a} ↔ {b}: cos={s:.3f}")

    # 2) frame → face track lookup (embedding 추출된 것만)
    ...
    for start, end, spk in audio_segments:
        ...
        # segment 구간 face track별 frame 카운트
        ...
        # 안전: best가 현재 speaker와 다르고, threshold 통과하고, 2위와 충분히 차이남
        if (best_spk != spk
