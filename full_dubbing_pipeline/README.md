...
**실측 예시 (Good Doctor 시리즈, 모두 full v178 env 베이스 + 영상별 추가)**:

공통 base (DIARIZATION_SWEEP_LOG.md line 333-440 + orchestrator.py line 1271 fix):
```bash
LATENTSYNC_RESUME=1                       # 핵심: vocals cache 재사용 (BS-RoFormer 비결정성 배제)
LATENTSYNC_VOCALS_NORMALIZE=0
LATENTSYNC_F0_GENDER=0
LATENTSYNC_DIARIZE_MERGE_THRESHOLD=0.60
LATENTSYNC_INTRASPK_SPLIT=1
LATENTSYNC_INTRASPK_MIN_SIM=0.50
LATENTSYNC_INTRASPK_REASSIGN_GAP=0.20
LATENTSYNC_INTRASPK_GAP=0.10
LATENTSYNC_INTRASPK_PASSES=3
LATENTSYNC_ERES2_MIN_SIM=0.30
LATENTSYNC_ERES2_MARGIN=0.05
LATENTSYNC_ERES2_SHORT_DUR=3.0
...
영상별 추가:
- **test4 (108초, 6명 화자)**: 위 base + `LATENTSYNC_OUTLIER_OFF=1` → 6 SPK (SPK_00~05) v195 baseline 라벨 완전 일치 ✓
- **test5 (86초, 6명 — 메인 4 + 배경 2)**: 위 base + `LATENTSYNC_OUTLIER_FAR_THRESH=0.70` + `FUSION_MIN_SPEAKER_RATIO=0.005` + `FUSION_MIN_SPEAKER_FRAMES=5` (OUTLIER 켬) → 7 SPK (GT 6 매핑 + 1 false positive)
- 영상에 따라 outlier 검출이 도움(test5) vs 방해(test4) — env 다름 불가피
