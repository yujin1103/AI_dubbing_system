#!/bin/bash
# test4 v34 — FACE_ID_CLUSTER_SHARE 60% → 40% (더 적극적 remap)
#
# v33 진단: SPK_01 share 51%, SPK_03 share 45% 둘 다 60% 미만이라 remap skip.
# v34: 40% 이상이면 발동 → 두 화자 모두 remap 후보 됨.
# 효과 예상:
#   - SPK_03 (8 groups) face cluster 17과 매칭 (45%) → cluster_17의 dominant
#     speaker로 흡수
#   - SPK_01 (7 groups) cluster_0와 매칭 (51%) → 흡수
# 위험:
#   - over-merge로 화자 수 < 6명 됨 (user GT는 6명)
#   - cluster의 dominant이 잘못된 화자일 가능성
# 가설 D는 이미 구현됨 (face_id remap 후 speaker_face_count도 자동 update)
# baseline: v33 + CLUSTER_SHARE 변경만

set -e
INPUT=/workspace/media/input/test4.mp4
RUN_ID=test4_v34_$(date +%Y%m%d_%H%M%S)
echo "==================================================="
echo "  test4 v34 - FACE_ID_CLUSTER_SHARE=0.40"
echo "  Run ID: $RUN_ID"
echo "==================================================="
...
# v34 CHANGE: cluster share 임계 40% (was default 60%)
export LATENTSYNC_FACE_ID_CLUSTER_SHARE=0.40
# face_id sim default (0.50) 유지
...
cd /opt/LatentSync
START=$(date +%s)
echo "[$(date '+%H:%M:%S')] 시작 (CLUSTER_SHARE=0.40)"

/opt/venv_lipsync/bin/python /workspace/orchestrator.py \
    --input "$INPUT" --name "test4_v34" --run-id "$RUN_ID" \
    --lang ko --content-type drama 2>&1 | tee /tmp/test4_v34_${RUN_ID}.log | \
...
echo
echo "=== 59-72s 영역 + SPK_05 ==="
SEG=$(ls -t /workspace/media/runs/test4_v34_*/meta/test4_v34_chunk_000_segments.json 2>/dev/null | head -1)
python3 << PYEOF
import json
...
for g in sorted(data["groups"], key=lambda x: x["group_start"]):
    if 58 <= g["group_start"] <= 73:
        ...
        print(f"  [{g['group_idx']:>2}] {g['speaker']} {g['group_start']:>6.2f}~{g['group_end']:>6.2f}s ({d:.2f}s) '{g.get('text','')[:30]}'")
print()
...
echo
echo "=== Auto metric v26 vs v31 vs v33 vs v34 ==="
...
python3 /workspace/tmp/eval_diarization_auto.py "$V26" "$V31" "$V33" "$SEG" 2>&1 | tail -25
