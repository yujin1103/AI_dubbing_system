...
RUN_ID=test4_v24_$(date +%Y%m%d_%H%M%S)

echo "==================================================="
echo "  test4 v24 - Face Recognition 임계 강화 + 추출 개선"
echo "  Run ID: $RUN_ID"
echo "==================================================="
...
export LATENTSYNC_FACE_ID_SIM=0.60                  # 0.50 → 0.60
...
/opt/venv_lipsync/bin/python /workspace/orchestrator.py \
    ...
    --run-id "$RUN_ID" \
    --lang ko \
    --content-type drama 2>&1 | tee /tmp/test4_v24_${RUN_ID}.log | \
...
RESULT=/workspace/media/output/test4_v24_ko_${RUN_ID}.mp4
if [ -f "$RESULT" ]; then
    echo "  결과: $RESULT"
    echo "  호스트: E:\\TTS_capstone\\media\\output\\test4_v24_ko_${RUN_ID}.mp4"
fi
echo "==================================================="
...
echo "[임베딩 추출률 (목표: 60%+)]"
grep "FaceID\] [0-9]*/[0-9]* face tracks" /tmp/test4_v24_${RUN_ID}.log 2>/dev/null
echo
echo "[face cluster 수 (목표: 6명에 가까움)]"
grep "face clusters" /tmp/test4_v24_${RUN_ID}.log 2>/dev/null
echo
echo "[skip된 cluster/speaker (안전장치 발동)]"
grep -E "cluster_[0-9]+ skip|remap skip" /tmp/test4_v24_${RUN_ID}.log 2>/dev/null | head -10
echo
echo "[적용된 speaker remap]"
grep -A 5 "speaker remap (face" /tmp/test4_v24_${RUN_ID}.log 2>/dev/null | head -10
echo
echo "[remap 결과 화자 수]"
grep "speaker remap 적용" /tmp/test4_v24_${RUN_ID}.log 2>/dev/null

echo
echo "=== Speaker 검증 ==="
SEG_JSON=$(ls -t /workspace/media/runs/test4_v24_*/meta/test4_v24_chunk_000_segments.json 2>/dev/null | head -1)
if [ -n "$SEG_JSON" ]; then
